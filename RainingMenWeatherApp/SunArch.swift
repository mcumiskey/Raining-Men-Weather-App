...
//
//  Created by Miles Cumiskey on 11/26/23.
// Arch motion with help via Ivo Vacek.
...
struct SunArch: View {
    @ObservedObject var aircraft = SunModel(from: .init(x: 0, y: 300), to: .init(x: 400, y: 300), control1: .init(x: 150, y: 250), control2: .init(x: 250, y: 250))
...
class SunModel: ObservableObject {
    ...
    var aircraft: some View {
        ...
        return Text("☀️").font(.largeTitle).rotationEffect(h).position(p)
    }
...
struct SunArch_Previews: PreviewProvider {
